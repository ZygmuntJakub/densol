...
Just heads up, I am an engineer with some background but totally new to Solana and the whole DeFi ecosystem. So if you see any mistakes or have any suggestions, please let me know.
...
SBF (Solana Binary Format) is the compilation target for Solana programs. It is executed by the Solana runtime (RBPF). The main limit I had to fight against was the heap size: 32 KB bump allocator. You can use `Vec`, `String` and other `alloc` types, but no filesystem, networking, or threads.
...
- **Break-even writes** — `(rent_saving × 1e6) / (overhead_CU × priority_fee)`; how many writes before CU cost catches up to rent savings. Uses priority fee = 1,000 µL/CU (`harmful` = compression expanded the data)
...
| repetitive | 512 B | 85 B | 6.02x | 4,840 | 10,277 | 5,437 | +2,971,920 | 546,490 |
| repetitive | 800 B | 86 B | 9.30x | 4,840 | 10,852 | 6,012 | +4,969,440 | 826,544 |
| json-like | 256 B | 147 B | 1.74x | 4,840 | 13,473 | 8,633 | +758,640 | 87,877 |
| json-like | 512 B | 148 B | 3.46x | 4,840 | 14,018 | 9,178 | +2,533,440 | 276,032 |
| json-like | 800 B | 149 B | 5.37x | 4,840 | 14,593 | 9,753 | +4,530,960 | 464,564 |
...
- **Break-even reads** — `(rent_saving × 1e6) / (overhead_CU × priority_fee)`; how many reads before CU cost catches up to rent savings. Uses priority fee = 1,000 µL/CU (`harmful` = compression expanded the data, no benefit)
...
| random | 8 KB | — | 1.00x | 83,444 | OOM | OOM | 0 | OOM |
| random | 10 KB | — | 1.00x | 103,951 | OOM | OOM | 0 | OOM |

### Conclusion

For structured data, compression saves hundreds of thousands to millions of lamports in rent deposits while the extra CU cost per operation is negligible (single-digit lamports at 1,000 µL/CU priority fee). Break-even is in the hundreds of thousands of operations -- practically unreachable. The only case where compression hurts is random/incompressible data, where LZ4 slightly expands the output.
