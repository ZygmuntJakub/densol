/**
 * Orca Whirlpools TickArray — on-chain compression benchmark (50-account sample)
 * ─────────────────────────────────────────────────────────────────────────────
 * What this does
 *   1. Loads inactive TickArray pubkeys from benchmark/orca/results/scan_results.json.
 ...
 *   4. Reports per-account results and averages; extrapolates to all inactive TickArrays.
 *
 * TickArray is 9,988 B (< 12 KB) → always uses storeRaw + compressStoredChunked.
 *
 * Prerequisites
 ...
 *   RPC_URL=https://mainnet.helius-rpc.com/?api-key=<KEY> yarn orca:bench
 */
...
describe("ORCA WHIRLPOOLS TICKARRAY BENCHMARK", () => {
  ...
  it(`compresses ${SAMPLE_SIZE} random inactive TickArray accounts on-chain and averages results`, async function () {
    ...
    console.log(`TickArray size from scan: ${scanData.tickArraySize?.toLocaleString() ?? "unknown"} B`);
    ...
    console.log(hr);
    console.log(`Extrapolation — all inactive Orca Whirlpools TickArrays on mainnet`);
    console.log(`  (source: benchmark/orca/scan.ts)`);
