/**
 * Marginfi (Project Zero) MarginfiAccount — on-chain compression benchmark (50-account sample)
 * ─────────────────────────────────────────────────────────────────────────────
 * What this does
 *   1. Loads inactive MarginfiAccount pubkeys from benchmark/marginfi/results/scan_results.json.
 ...
 *   4. Reports per-account results and averages; extrapolates to all inactive accounts.
 *
 * MarginfiAccount is 2312 B (< 12 KB) → always uses storeRaw + compressStoredChunked.
 *
 * Prerequisites
 ...
 *   RPC_URL=https://mainnet.helius-rpc.com/?api-key=<KEY> yarn marginfi:bench
 */
...
async function benchmarkAccount(
  ...
  // Compress on-chain with ChunkedLZ4
...
describe("MARGINFI MARGINFIACCOUNT BENCHMARK", () => {
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);
  const program = anchor.workspace.CompressBench as Program<CompressBench>;

  it(`compresses ${SAMPLE_SIZE} random inactive MarginfiAccount accounts on-chain and averages results`, async function () {
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      try {
        ...
        const r = await benchmarkAccount(pubkey, Buffer.from(info.data), provider, program, payer);
        results.push(r);
    ...
    console.log(hr);
    console.log(`Extrapolation — all inactive Marginfi MarginfiAccounts on mainnet`);
    console.log(`  (source: benchmark/marginfi/scan.ts)`);
