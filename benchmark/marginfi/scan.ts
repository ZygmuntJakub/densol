/**
 * scan.ts — count inactive Marginfi (Project Zero) MarginfiAccount accounts.
 *
 * Usage:
 *   RPC_URL=https://... yarn ts-node --transpile-only benchmark/marginfi/scan.ts
 *
 * Env vars:
 ...
 *
 * Single-phase approach (simpler than AMM scans — no parent/child relationship):
 *   Fetch all MarginfiAccount pubkeys, check each for inactivity via
 *   getSignaturesForAddress. Inactive accounts are candidates for compression.
 *
 * Account layout (zero_copy, bytemuck::Pod, 2304 B + 8 B discriminator = 2312 B):
 *   [8B discriminator][32B group][32B authority][1728B lending_account][...rest]
 *   Inactive accounts have all 16 Balance slots zeroed → highly compressible.
 *
 * Discriminator: hardcoded [67, 178, 130, 109, 126, 114, 28, 42] (not sha256-derived).
 */
...
// Hardcoded discriminator from type-crate/src/constants.rs — NOT sha256-derived
const MARGINFI_ACCOUNT_DISCRIMINATOR = Buffer.from([67, 178, 130, 109, 126, 114, 28, 42]);
const MARGINFI_ACCOUNT_DISCRIMINATOR_B58 = utils.bytes.bs58.encode(MARGINFI_ACCOUNT_DISCRIMINATOR);

const MARGINFI_ACCOUNT_SIZE = 2312; // 8 discriminator + 2304 struct
...
function printSummary(results: Results, totalAccounts: number, accountSize: number) {
  ...
  console.log("\n\n=== RESULTS ===");
  console.log(`Total MarginfiAccounts:     ${totalAccounts.toLocaleString()}`);
  ...
  console.log(`  Account size:      ~${accountSize > 0 ? accountSize.toLocaleString() : "unknown"} B`);
  ...
  console.log(`  (exact rent savings: run benchmark/marginfi/benchmark.ts)`);
...
async function main() {
  ...
  console.log(`Discriminator:     ${MARGINFI_ACCOUNT_DISCRIMINATOR_B58}`);
  console.log(`Account size:      ${MARGINFI_ACCOUNT_SIZE} B`);
  console.log();

  // Fetch all MarginfiAccount pubkeys
  console.log("Fetching all MarginfiAccount accounts...");
  const allAccounts = await connections[0].getProgramAccounts(MARGINFI_PROGRAM, {
    filters: [{ memcmp: { offset: 0, bytes: MARGINFI_ACCOUNT_DISCRIMINATOR_B58 } }],
    dataSlice: { offset: 0, length: 0 },
  });

  const totalAccounts = allAccounts.length;
  console.log(`Total MarginfiAccounts on mainnet: ${totalAccounts.toLocaleString()}`);
    ...
    while (true) {
      ...
      if (done % 500 === 0) {
        const inactiveKeys = Object.entries(results)
          ...
          .map(([k]) => k);
        saveResults(results, totalAccounts, inactiveKeys, MARGINFI_ACCOUNT_SIZE);
      }
    }
  }
  ...
  saveResults(results, totalAccounts, inactiveKeys, MARGINFI_ACCOUNT_SIZE);
  printSummary(results, totalAccounts, MARGINFI_ACCOUNT_SIZE);
}
...
function onExit() {
  const inactiveKeys = Object.entries(_results)
    .filter(([, s]) => s === "inactive" || s === "new")
    .map(([k]) => k);
  saveResults(_results, _totalAccounts, inactiveKeys, MARGINFI_ACCOUNT_SIZE);
  printSummary(_results, _totalAccounts, MARGINFI_ACCOUNT_SIZE);
}
