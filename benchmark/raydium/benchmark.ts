/**
 * Raydium CLMM TickArrayState — on-chain compression benchmark (50-account sample)
 * ─────────────────────────────────────────────────────────────────────────────
 * What this does
 *   1. Loads inactive TickArray pubkeys from benchmark/raydium/results/scan_results.json.
 ...
 *   4. Reports per-account results and averages; extrapolates to all inactive TAs.
 *
 * Why ChunkedLZ4, not plain LZ4?
 *   TickArrayState is ~10 KB. Plain LZ4 heap peak ≈ 2× input ≈ 20 KB, uncomfortably
 *   close to the 32 KB SBF limit. ChunkedLZ4<4096> processes 4 KB at a time —
 *   heap peak stays at ~4 KB. Account<DataStore> at 10 KB is fine for Anchor
 *   deserialization (OOM limit is ~12 KB), so we can use compressStoredChunked
 *   rather than the large-account variant.
 *
 * Prerequisites
 *   anchor build && anchor deploy  (or anchor test once to deploy)
 *
 * Usage
 *   RPC_URL=https://mainnet.helius-rpc.com/?api-key=<KEY> yarn raydium:bench
 *
 * Env vars
...
async function benchmarkTickArray(
  pubkey: string,
  bytes: Buffer,
  ...
  // Upload TickArray bytes in 900 B chunks
...
describe("RAYDIUM CLMM TICKARRAY BENCHMARK", () => {
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);
  const program = anchor.workspace.CompressBench as Program<CompressBench>;

  it(`compresses ${SAMPLE_SIZE} random inactive TickArray accounts on-chain and averages results`, async function () {
    this.timeout(1_800_000); // 30 min for 50 accounts
    ...
    const inactiveTA: string[] = scanData.tickArrays ?? [];
    assert(inactiveTA.length > 0, "No inactive TickArrays in scan results — run scan.ts first");

    const sample = shuffle(inactiveTA).slice(0, SAMPLE_SIZE);
    console.log(
      `\nSampled ${sample.length} TickArrays from ${inactiveTA.length.toLocaleString("en")} inactive`
    );
    if (scanData.tickArraySize > 0) {
      console.log(`TickArrayState size from scan: ${scanData.tickArraySize.toLocaleString()} B`);
    }

    // ── 2. Setup shared payer ─────────────────────────────────────────────
    const mainnet = new Connection(MAINNET_RPC, "confirmed");
    // 5 SOL covers upload rent growth for 50 accounts (~0.072 SOL each) + tx fees
    const payer = await fundedPayer(provider.connection, 5 * LAMPORTS_PER_SOL);
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      try {
        ...
        const r = await benchmarkTickArray(pubkey, Buffer.from(info.data), provider, program, payer);
        results.push(r);
    ...
    const totalInactiveTA = inactiveTA.length;
    const estTotalSOL = totalInactiveTA * avgRentSOL;
    ...
    console.log(`  note: production instruction must explicitly refund freed lamports to pool owner`);
    console.log(hr);
    console.log(`Extrapolation — all inactive Raydium CLMM TickArrays on mainnet`);
    console.log(`  (source: benchmark/raydium/scan.ts)`);
    console.log(`  inactive TickArrays: ${totalInactiveTA.toLocaleString("en")}`);
