/**
 * Kamino Lending Obligation — on-chain compression benchmark (50-account sample)
 * ─────────────────────────────────────────────────────────────────────────────
 * What this does
 *   1. Loads inactive Obligation pubkeys from benchmark/kamino/results/scan_results.json.
 ...
 *   4. Reports per-account results and averages; extrapolates to all inactive Obligations.
 *
 * Obligation is 3,344 B (< 12 KB) → always uses storeRaw + compressStoredChunked.
 *
 * Prerequisites
 ...
 *   RPC_URL=https://mainnet.helius-rpc.com/?api-key=<KEY> yarn kamino:bench
 */
...
async function benchmarkObligation(
  pubkey: string,
  bytes: Buffer,
...
describe("KAMINO LENDING OBLIGATION BENCHMARK", () => {
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);
  const program = anchor.workspace.CompressBench as Program<CompressBench>;

  it(`compresses ${SAMPLE_SIZE} random inactive Obligation accounts on-chain and averages results`, async function () {
    ...
    const inactiveAccounts: string[] = scanData.accounts ?? [];
    assert(inactiveAccounts.length > 0, "No inactive accounts in scan results — run scan.ts first");

    const sample = shuffle(inactiveAccounts).slice(0, SAMPLE_SIZE);
    console.log(`\nSampled ${sample.length} accounts from ${inactiveAccounts.length.toLocaleString("en")} inactive`);
    console.log(`Account size from scan: ${scanData.accountSize?.toLocaleString() ?? "unknown"} B`);
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      try {
        ...
        const r = await benchmarkObligation(pubkey, Buffer.from(info.data), provider, program, payer);
        results.push(r);
    ...
    const estTotalSOL = totalInactive * avgRentSOL;

    console.log(`\n${hr}`);
    console.log(`AVERAGES  (${results.length}/${sample.length} succeeded, ${errors} skipped/errored)`);
    console.log(hr);
    console.log(`Raw size:          ${avg(rawSizes).toFixed(0)} B avg  (min ${Math.min(...rawSizes)} B, max ${Math.max(...rawSizes)} B)`);
    console.log(`Compressed size:   ${avg(compSizes).toFixed(0)} B avg  (min ${Math.min(...compSizes)} B, max ${Math.max(...compSizes)} B)`);
    ...
    console.log(hr);
    console.log(`Extrapolation — all inactive Kamino Lending Obligations on mainnet`);
    console.log(`  (source: benchmark/kamino/scan.ts)`);
    console.log(`  inactive accounts: ${totalInactive.toLocaleString("en")}`);
